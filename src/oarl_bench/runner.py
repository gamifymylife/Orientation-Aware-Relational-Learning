...
def run_episode(cfg: BenchmarkConfig, seed: int, policy: str) -> dict:
    ...
    for step in range(1, cfg.budget + 1):
        ...
        if choice.use_transport:
            rep, ca, y_can, sigma_can = world.transport_observation(choice.orientation, choice.intervention, y)
            posterior = update_posterior(posterior, world.means[:, rep, ca], sigma_can, y_can)
