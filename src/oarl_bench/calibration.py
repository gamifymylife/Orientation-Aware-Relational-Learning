...
def calibrate(
    ...
    bkey = {
        (r.seed, r.regime, r.perturbation_scale): r
    ...
    for lam, gam in itertools.product(lambdas, gammas):
        ...
        for regime, perturb in conditions:
            cfg = replace(
                ...
                lambda_stability=float(lam),
                gamma_cost=float(gam),
            )
            for seed in seeds:
        ...
        # Hard preregistered reliability gate: cost savings are inadmissible if
        # they buy more than a 2 percentage-point degradation in identification
        # success or final correctness relative to generic OED.
    ...
    table = pd.DataFrame(rows).sort_values(
        ["objective", "median_paired_penalized_cost_reduction"], ascending=False
