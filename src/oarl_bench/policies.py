...
def _cost_tiebreak_choice(world: RelationalWorld, scores: np.ndarray, valid_mask: np.ndarray | None = None) -> Choice:
    ...
    # Exact-equivalent actions should have equal IG up to floating point noise.
...
def _structured_choice(
    world: RelationalWorld,
    posterior: np.ndarray,
    cfg: BenchmarkConfig,
    cheapest_equivalent: bool,
) -> Choice:
    # One admissible representative per equivalence class.
    active_classes = []
    reps = []
    for c, rep in enumerate(world.class_representative):
        ...
        # canonical likelihood always lives at the declared representative;
        # the representative itself is expected admissible for exact classes.
        reps.append(int(rep))
...
def choose_experiment(
    ...
    if policy == "generic_true_cost_tiebreak":
        ig = _ig_matrix(world, posterior, cfg)
        return _cost_tiebreak_choice(world, ig, world.true_admissible)

    if policy == "generic_cost_tiebreak":
        ig = _ig_matrix(world, posterior, cfg)
        return _cost_tiebreak_choice(world, ig, world.admissible)
    ...
    # v0.2 policies kept for backwards compatibility.
