"""v0.7 decision-complete complementary orientation search.

Ported without algorithmic changes from the frozen v0.6 Complementary Orientation
Gate in draft PR #12. The module uses only candidate-mechanism predictions; the
hidden true mechanism is used only to generate observations and score outcomes.
"""
...
def expected_utility(world: ComplementarityWorld, evidence: dict[int, int], orientation: int, *, target: str = "decision") -> float:
    idx = compatible(world, evidence)
    if len(idx) == 0:
        return 0.0
...
def pair_synergy(world: ComplementarityWorld, evidence: dict[int, int], a: int, b: int) -> float:
    idx = compatible(world, evidence)
    if len(idx) == 0:
        return 0.0
...
def run_policy(world: ComplementarityWorld, policy: str, *, seed: int = 0) -> dict:
    ...
    return {"world": world.name, "policy": policy, "probes": probes, "cost": cost, "planning_evals": planning_evals, "decision": d, "true_decision": true_d, "correct": d == true_d, "unknown": d == UNKNOWN, "decision_complete": d != UNKNOWN, "orientations_used": tuple(sorted(evidence))}
...
def _world(name, cols, decisions, *, true_h=0, costs=None, classes=None):
    ...
    return ComplementarityWorld(name=name, responses=r, decisions=np.asarray(decisions, dtype=int), costs=np.asarray(costs, dtype=float), equivalence_class=np.asarray(classes, dtype=int), true_h=int(true_h))
...
def benchmark_worlds(true_h: int = 0) -> list[ComplementarityWorld]:
    ...
    worlds = [
        ...
        _world("exact_redundancy", [xor_decision, [7, 8, 8, 7], x, y], xor_decision, true_h=true_h, classes=[0, 0, 1, 2]),
        ...
        _world("fundamental_insufficiency", [constant, [0, 0, 1, 1], [2, 2, 3, 3]], [0, 1, 0, 1], true_h=true_h),
    ]
    bits = np.array([[i >> 2 & 1, i >> 1 & 1, i & 1] for i in range(8)], dtype=int)
    parity = np.bitwise_xor.reduce(bits, axis=1)
    worlds.append(ComplementarityWorld(name="higher_order_complementarity", responses=bits, decisions=parity, costs=np.ones(3), equivalence_class=np.arange(3), true_h=min(true_h, 7)))
    return worlds


def run_gate(seeds: Iterable[int] = range(16)) -> list[dict]:
    policies = ["fixed", "random", "exhaustive", "greedy_feature", "active_decision", "two_step", "oarl_no_synergy", "oarl_no_transport", "oarl"]
    rows = []
    for seed in seeds:
        for h in range(4):
