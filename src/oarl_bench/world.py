...
class RelationalWorld:
    ...
    def observe(self, orientation: int, intervention: int, rng: np.random.Generator) -> float:
        ...
        if self.perturbation_scale > 0:
            mu = mu + rng.normal(0.0, self.perturbation_scale * self.stability[orientation] * sigma)
    ...
    def transport_observation(self, orientation: int, intervention: int, y_raw: float) -> tuple[int, int, float, float]:
...
def _apply_equivalence_structure(cfg: BenchmarkConfig, rng: np.random.Generator, means: np.ndarray, sigma: np.ndarray):
    H, O, A = means.shape
    ...
    for o in range(O):
        ...
        if o == reps[c]:
            perm = np.arange(A, dtype=int); s = 1.0; b = 0.0
        else:
        ...
        mapping[o] = perm; scale[o] = s; offset[o] = b
...
def _false_split(orientation_class, reps, mapping, scale, offset, rate, rng):
    if rate <= 0:
        return orientation_class, reps, mapping, scale, offset
    oc = orientation_class.copy(); reps_list = list(map(int, reps)); mp = mapping.copy(); sc = scale.copy(); off = offset.copy()
    ...
    A = mp.shape[1]
    for o in chosen:
        new_c = len(reps_list); oc[o] = new_c; reps_list.append(int(o))
        mp[o] = np.arange(A, dtype=int); sc[o] = 1.0; off[o] = 0.0
    return oc, np.asarray(reps_list, dtype=int), mp, sc, off


def _false_merge(orientation_class, reps, rate, rng):
    C = len(reps)
    if rate <= 0 or C <= 1:
    ...
    for o, c in enumerate(orientation_class):
        new_oc[o] = new_id[int(parent[int(c)])]
    new_reps = np.asarray([int(reps[root]) for root in roots], dtype=int)
    return new_oc, new_reps


def _corrupt_metadata(cfg, rng, orientation_class, reps, mapping, scale, offset, admissible, true_admissible):
    oc, rp, mp, sc, off = _false_split(orientation_class, reps, mapping, scale, offset, cfg.metadata_false_split_rate, rng)
    oc, rp = _false_merge(oc, rp, cfg.metadata_false_merge_rate, rng)
    adm = admissible.copy()
    invalid = np.where(~true_admissible)[0]; valid = np.where(true_admissible)[0]
    if cfg.admissibility_false_positive_rate > 0 and len(invalid):
        ...
        if n > 0: adm[rng.choice(invalid, size=n, replace=False)] = True
    if cfg.admissibility_false_negative_rate > 0 and len(valid):
        ...
        if n > 0 and len(candidates): adm[rng.choice(candidates, size=min(n, len(candidates)), replace=False)] = False
...
def generate_world(cfg: BenchmarkConfig, seed: int) -> RelationalWorld:
    cfg.validate(); rng = np.random.default_rng(seed)
    ...
    raw_exposure = rng.beta(1.4, 1.4, size=O); raw_exposure[0] = 0.0
    ...
    stability = cfg.exposure_instability_correlation * (0.2 + 2.2 * exp_norm) + (1.0 - cfg.exposure_instability_correlation) * independent
    ...
    if cfg.world_regime == "standard":
        means[:, 0, :] = baseline[0, :][None, :]; stability[0] = min(stability[0], 0.25)
    elif cfg.world_regime == "no_orientation_value":
        template = means[:, 1 if O > 1 else 0, :].copy()
        for o in range(O): means[:, o, :] = template
        exposure[:] = float(np.mean(exposure[1:])) if O > 1 else 1.0; stability[:] = 1.0; cost[:] = 1.0
    elif cfg.world_regime == "informative_unstable":
        means[:, 0, :] = baseline[0, :][None, :]; stability[0] = 0.2
        if O > 1:
            center = means[:, 1, :].mean(axis=0, keepdims=True)
            means[:, 1, :] = center + 3.5 * (means[:, 1, :] - center)
            exposure[1] = max(exposure.max(), cfg.exposure_scale * 2.5); stability[1] = 8.0
        if O > 2: stability[2:] = np.clip(stability[2:], 0.2, 1.4)
    elif cfg.world_regime == "orientation_exclusive":
        groups = np.arange(H) // 2; group_signal = rng.normal(size=(groups.max() + 1, A))
        means[:, 0, :] = group_signal[groups]; exposure[0] = 0.5; stability[0] = 0.3
        if O > 1:
            ...
            means[:, 1, :] = group_base + pair_sign * sep; exposure[1] = max(exposure[1], cfg.exposure_scale); stability[1] = min(stability[1], 0.8)
    elif cfg.world_regime == "equivalent_redundancy":
        means[:, 0, :] = baseline[0, :][None, :]
        orientation_class, reps, mapping, scale, offset, admissible = _apply_equivalence_structure(cfg, rng, means, nominal_sigma)
    elif cfg.world_regime == "asymmetric_invalid":
        means[:, 0, :] = baseline[0, :][None, :]
        n_invalid = max(1, int(round(O * cfg.invalid_orientation_fraction)))
        invalid_idx = np.arange(1, min(O, n_invalid + 1), dtype=int); admissible[invalid_idx] = False
        for o in invalid_idx:
            center = means[:, o, :].mean(axis=0, keepdims=True)
            means[:, o, :] = center + 5.0 * (means[:, o, :] - center)
            nominal_sigma[o, :] *= 0.7; stability[o] = max(stability[o], 5.0)

    actual_means = means.copy()
    if cfg.world_regime == "asymmetric_invalid":
        invalid_idx = np.where(~admissible)[0]; wrong_map = (np.arange(H) + 1) % H
        for o in invalid_idx: actual_means[:, o, :] = means[wrong_map, o, :]

    true_oc = orientation_class.copy(); true_reps = reps.copy(); true_mapping = mapping.copy(); true_scale = scale.copy(); true_offset = offset.copy(); true_admissible = admissible.copy()
    true_h = int(rng.integers(H))
    corruption_rng = np.random.default_rng(seed * 1000003 + 77031)
    orientation_class, reps, mapping, scale, offset, admissible = _corrupt_metadata(cfg, corruption_rng, orientation_class, reps, mapping, scale, offset, admissible, true_admissible)

    return RelationalWorld(means=means, actual_means=actual_means, nominal_sigma=nominal_sigma, stability=stability, cost=cost, exposure=exposure, true_h=true_h, perturbation_scale=cfg.perturbation_scale, seed=seed, regime=cfg.world_regime, orientation_class=orientation_class, class_representative=reps, to_canonical_intervention=mapping, transform_scale=scale, transform_offset=offset, admissible=admissible, true_orientation_class=true_oc, true_class_representative=true_reps, true_to_canonical_intervention=true_mapping, true_transform_scale=true_scale, true_transform_offset=true_offset, true_admissible=true_admissible)
