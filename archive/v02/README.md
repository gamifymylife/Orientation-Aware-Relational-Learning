# Orientation-Aware Relational Learning Benchmark v0.2
...
Reliability-constrained calibration selected `lambda_stability=0` and `gamma_cost=0`. On 500 disjoint held-out worlds, frozen Full OARL and Generic OED were therefore exactly equal (0.0% paired penalized-C95 advantage, bootstrap interval [0.0%, 0.0%]). This is a negative result for the current additive stability/cost scoring rule, while the benchmark still strongly supports the narrower claim that changing orientation can expose mechanism distinctions inaccessible from a fixed boundary. See `V02_CONFIRMATORY_REPORT.md`.

## v0.2 confirmatory protocol

v0.2 separates calibration from confirmation. `lambda_stability` and `gamma_cost` are swept only on calibration seeds, frozen, and then evaluated on disjoint held-out seeds. The confirmatory suite adds three kill-controls: `no_orientation_value`, `informative_unstable`, and `orientation_exclusive`. It also adds `penalized_c95` so a policy that fails to reach 95% confidence cannot appear efficient merely because it selected cheap experiments.
...
Outputs include the frozen parameters, calibration sweep, 500-world held-out results, paired bootstrap comparisons, and a standalone HTML dashboard.
