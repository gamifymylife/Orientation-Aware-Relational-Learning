...
def info_gain_proxy(
    ...
    If the posterior-induced distribution of means were Gaussian, this would
    equal the mutual information of a scalar Gaussian channel.
    """
    m = float(np.sum(posterior * means_h))
    var = float(np.sum(posterior * (means_h - m) ** 2))
    return 0.5 * float(np.log1p(var / (sigma * sigma)))

def info_gain_quadrature(
    posterior: np.ndarray,
    means_h: np.ndarray,
    sigma: float,
    points: int = 12,
    ...
    """
    Numerically estimate I(H;Y) for a discrete hypothesis variable and
    homoscedastic one-dimensional Gaussian observation channel.
    """
    ...
    for h, ph in enumerate(posterior):
        ...
        # log p(y) via stable logsumexp over hypotheses.
        diffs = (ys[:, None] - means_h[None, :]) / sigma
        log_components = (
            np.log(posterior[None, :])
            + const
            - 0.5 * diffs ** 2
        )
...
def information_gain(
    ...
    if mode == "quadrature":
        return info_gain_quadrature(
            posterior, means_h, sigma, points=quadrature_points
        )
