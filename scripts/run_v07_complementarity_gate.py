"""Run the preserved v0.7 synthetic complementary-orientation gate."""
...
OUT = Path("evidence/v07/synthetic")
OUT.mkdir(parents=True, exist_ok=True)
rows = run_gate(range(32))

with (OUT / "v07_results.csv").open("w", newline="") as f:
...
with (OUT / "v07_summary.csv").open("w", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=list(summary[0]))
    writer.writeheader()
    writer.writerows(summary)

resolvable = {"single_view", "exact_redundancy", "pure_complementarity", "misleading_similarity", "higher_order_complementarity"}
...
(OUT / "v07_headline.json").write_text(json.dumps(headline, indent=2) + "\n")
