...
def test_false_splits_only_increase_declared_class_count():
    base = BenchmarkConfig(world_regime="equivalent_redundancy", n_orientations=16, n_equivalence_classes=4, n_interventions=8)
...
def test_false_merge_reduces_declared_class_count_below_truth():
    cfg = BenchmarkConfig(world_regime="equivalent_redundancy", n_orientations=24, n_equivalence_classes=6, n_interventions=8, metadata_false_merge_rate=0.5)
...
def test_false_positive_admissibility_reopens_some_invalid_orientations():
    cfg = BenchmarkConfig(world_regime="asymmetric_invalid", n_orientations=16, invalid_orientation_fraction=0.25, admissibility_false_positive_rate=0.5)
...
def test_false_negative_admissibility_closes_some_valid_orientations():
    cfg = BenchmarkConfig(world_regime="standard", n_orientations=16, admissibility_false_negative_rate=0.5)
    w = generate_world(cfg, 45)
    assert np.sum((~w.admissible) & w.true_admissible) > 0


def test_zero_metadata_noise_preserves_true_structure():
    cfg = BenchmarkConfig(world_regime="equivalent_redundancy", n_orientations=16, n_equivalence_classes=4)
