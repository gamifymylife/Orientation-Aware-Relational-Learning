...
def run_one(paths, size, net, repeats=100):
    ...
    for rep in range(repeats):
        rng=np.random.default_rng(20260831+1000*net+rep+100000*size)
...
def bootstrap_effects(effects, B=10000, seed=20260831):
    rng=np.random.default_rng(seed)
    vals=np.asarray(effects,float)
    boots=[]
    for _ in range(B):
        boots.append(np.median(rng.choice(vals,len(vals),replace=True)))
    return float(np.median(vals)), float(np.quantile(boots,.025)), float(np.quantile(boots,.975))
...
def main():
    ...
    med,lo,hi=bootstrap_effects(effects)
    ko_wins=sum(pairdf.query("modality=='KO'").delta_auprc>0)
    kd_wins=sum(pairdf.query("modality=='KD'").delta_auprc>0)
    ratios=sum(pairdf.preserved_ratio>1)
    verdict=(ko_wins>=4 and kd_wins>=4 and med>0 and lo>0 and ratios>=3)
    summary={
        ...
        "bootstrap95":[lo,hi],
        "preserved_comparisons_above_random_ratio":int(ratios),
